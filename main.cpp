...
void print (int heap[101], string connection, int i);
void display (int heap[101], int heapSize, bool added);
void printCmds (bool added);
...
int main () {
  ...
  while (isRunning) {
    ...
    else if (cmd == "DISPLAY") { display (heap, heapSize, added); }
    else if (cmd == "QUIT") { isRunning = false; }
    else { cout << "Huh?" << endl; }    
  }
...
void printCmds (bool added) {
  ...
  if (add) cout << "- ADD -- add numbers to the heap" << endl;
